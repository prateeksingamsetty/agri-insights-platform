...
def select_dropdown_option(main_div_id, option_to_be_selected):
    ...
    while retries > 0:
        try:
            ...
            div_element.click()
            dropdown_options = wait.until(EC.visibility_of_all_elements_located((By.CSS_SELECTOR, f"#{main_div_id} .chosen-results li")))
...
prefs = {"download.default_directory": "/usr/Public"}  # Adjust path
...
try:
    ...
    select_dropdown_option("market_types_by_market_types_chosen", "Shipping Point")
    ...
    select_multiple_dropdown_options("commodities_by_market_types_section_2_chosen", {"Tomatoes": True})
    select_multiple_dropdown_options("locations_by_market_types_section_2_chosen", {"Atlanta (Forest Park), Georgia": True, "Columbia, South Carolina": True, "Miami, Florida": True, "Orlando (Oviedo), Florida": True, "Raleigh, North Carolina": True})
    ...
    push_data_to_mongodb(json_data, "Agri_Insights", "tomatoPrices", MONGODB_CONNECTION_STRING)
